...
def calHold(change_dict):
    ...
    for index in range(len(change_dict['BTCUSDT'])):
        ...
        if numpy.isnan(BTCChange) or numpy.isnan(ETHChange):
            hold_result[index] = [date, BTCChange, ETHChange, 'BTCUSDT'];
        ...
        # 并非两者都<0时，且以太坊涨得多
            ...
            #print('买入以太坊')
            hold_result[index] = [date, BTCChange, ETHChange,'ETHUSDT']
...
for N in range(1,21):
    ...
    df.columns = ['UTC+8时间','BTC当日涨跌幅', 'ETH当日涨跌幅', '持仓']
    ...
    df.columns = ['UTC+8时间','BTC当日涨跌幅', 'ETH当日涨跌幅','持仓','持仓数量','持仓U', '持仓净值'];
    df.to_csv(csv, encoding='utf_8_sig', index=None);
